...
export const getUserMeetingsService = async (
  ...
  if (filter === MeetingFilterEnum.CANCELLED) {
...
export const createMeetingBookingForGuestService = async (
  ...
  if (!Object.values(EventLocationEnumType).includes(event.locationType)) {
    throw new BadRequestException("Event location type is invalid");
  }
  ...
  if (!meetingIntegration) {
    throw new BadRequestException("No integration found for event");
  }
  ...
  if (event.locationType === EventLocationEnumType.GOOGLE_MEET_AND_CALENDAR) {
    const { calendar, calendarType } = await getCalendarClient(
      ...
      meetingIntegration.expiration_date,
    );
  ...
  switch (app_type) {
    ...
    default:
      throw new BadRequestException(
        `Unsupported calendar provider: ${app_type}`,
      );
  }
}
  ...
  try {
    ...
    if (calendarIntegration) {
      const { calendar, calendarType } = await getCalendarClient(
        ...
        calendarIntegration.expiration_date,
      );
      switch (calendarType) {
        case IntegrationAppTypeEnum.GOOGLE_MEET_AND_CALENDAR:
